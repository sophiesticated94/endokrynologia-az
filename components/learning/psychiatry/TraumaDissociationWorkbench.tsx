...
export function TraumaDissociationWorkbench({
  ...
  const renderFrameworkCard = (fw: FrameworkEvaluation) => (
    <div className="rounded-xl border border-border/80 bg-card p-3 text-xs shadow-sm">
      <div className="flex items-center justify-between border-b pb-2 mb-2">
        ...
        <span
          className={`rounded-full px-2 py-0.5 text-[10px] font-semibold ${
            fw.compatibility === 'meets'
              ...
              : fw.compatibility === 'insufficient_information'
        ...
        >
          {fw.compatibility === 'meets'
            ...
            ? 'Częściowo / Możliwe'
            : fw.compatibility === 'insufficient_information'
            ? 'Niewystarczające dane / Wymaga badań'
            : 'Nie spełnia'}
        </span>
      </div>
      ...
      {fw.unresolvedExclusions && fw.unresolvedExclusions.length > 0 && (
        <div className="rounded-md bg-amber-50 p-2 text-[11px] text-amber-800 dark:bg-amber-950/40 dark:text-amber-300 mb-2">
          <span className="font-semibold">Nierozstrzygnięte wykluczenia (wymaga ukończenia badań): </span>
          {fw.unresolvedExclusions.join('; ')}
        </div>
      )}
      {fw.confirmedExclusions && fw.confirmedExclusions.length > 0 && (
        <div className="rounded-md bg-rose-50 p-2 text-[11px] text-rose-800 dark:bg-rose-950/40 dark:text-rose-300 mb-2">
          <span className="font-semibold">Potwierdzone wykluczenia wyjaśniające objawy: </span>
          {fw.confirmedExclusions.join('; ')}
        </div>
      )}
      {(!fw.unresolvedExclusions && !fw.confirmedExclusions && fw.exclusions.length > 0) && (
