...
export const TraumaDissociationInputSchema = z.object({
  ...
  traumaAvoidance: z.enum(['none', 'internal', 'external', 'both', 'unassessed']).optional(),
  persistentCurrentThreat: z.enum(['none', 'hypervigilance', 'exaggerated_startle', 'both', 'unassessed']).optional(),
  negativeSelfConcept: z.enum(['none', 'persistent_shame_guilt', 'worthlessness_failure', 'trauma_related_negative_identity', 'unassessed']).optional(),
  affectRegulation: z.object({
    reactiveLability: z.enum(['none', 'mild', 'marked', 'unassessed']).optional(),
    persistentDysregulation: z.enum(['none', 'hyperactivation', 'hypoactivation_numbing', 'mixed', 'unassessed']).optional(),
  }).strict().optional(),
  relationalDisturbance: z.object({
    sustainedDifficultyWithCloseness: z.enum(['none', 'present', 'unassessed']).optional(),
    persistentDetachmentOrAlienation: z.enum(['none', 'present', 'unassessed']).optional(),
    unstableIntenseRelationships: z.enum(['none', 'present', 'unassessed']).optional(),
    abandonmentSensitivity: z.enum(['none', 'present', 'marked', 'unassessed']).optional(),
  }).strict().optional(),
  psychosisAxes: z.object({
    realityTesting: z.enum(['intact', 'partially_impaired', 'markedly_impaired', 'unassessed']).optional(),
    delusions: z.object({
      presence: z.enum(['none', 'suspected', 'present', 'unassessed']).optional(),
      organization: z.enum(['none', 'poorly_formed', 'systematized', 'bizarre', 'unassessed']).optional(),
      conviction: z.enum(['none', 'low', 'moderate', 'high', 'unassessed']).optional(),
      systematization: z.enum(['none', 'non_systematized', 'systematized', 'unassessed']).optional(),
      themes: z.array(z.string()).optional(),
    }).strict().optional(),
    formalThoughtDisorder: z.enum(['none', 'mild', 'marked', 'unassessed']).optional(),
    behavioralDisorganization: z.enum(['none', 'mild', 'marked', 'unassessed']).optional(),
    negativeSymptoms: z.object({
      ...
      affectiveFlattening: z.boolean().optional(),
      asociality: z.boolean().optional(),
    }).strict().optional(),
    functionalDecline: z.enum(['none', 'possible', 'clear', 'unassessed']).optional(),
    longitudinalCourse: z.enum(['unknown', 'brief', 'episodic', 'persistent', 'progressive', 'unassessed']).optional(),
    moodRelation: z.enum(['independent', 'only_during_mood_episode', 'mixed_or_unclear', 'unassessed']).optional(),
    moodEpisodeRelation: z.enum(['independent', 'only_during_mood_episode', 'mixed_or_unclear', 'unassessed']).optional(),
    substanceOrMedicalContext: z.enum(['none_known', 'possible', 'likely', 'unassessed']).optional(),
    substanceContext: z.enum(['none_known', 'possible_trigger', 'direct_cause', 'unassessed']).optional(),
  }).strict().optional(),
  ...
  neurologicalFeatures: z.object({
    episodicPattern: z.enum(['none', 'non_stereotyped', 'stereotyped', 'unassessed']).optional(),
    episodeDuration: z.enum(['seconds', 'minutes', 'hours', 'variable', 'unknown', 'unassessed']).optional(),
    aura: z.object({
    ...
    impairedAwareness: z.enum(['none', 'possible', 'clear', 'unassessed']).optional(),
    witnessedAutomatisms: z.enum(['none', 'possible', 'clear', 'unassessed']).optional(),
    ...
    witnessHistory: z.enum(['unavailable', 'available_non_supportive', 'available_supportive', 'unassessed']).optional(),
    focalNeurologicalDeficits: z.enum(['none', 'possible', 'present', 'unassessed']).optional(),
  ...
  culturalContext: z.object({
    culturallySanctionedExperience: z.boolean().optional(),
    religiousOrSpiritualContext: z.boolean().optional(),
...
export const TraumaDissociationPresetSchema = z.object({
  ...
  input: TraumaDissociationInputSchema.optional(),
}).strict();
