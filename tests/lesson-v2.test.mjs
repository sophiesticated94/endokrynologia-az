...
test('all 178 endocrinology lessons use the complete v2 learning structure', () => {
  assert.equal(lessons.length, 178);
  assert.equal(Object.keys(lessonExperiences).length, 178);
  const ids = new Set();
...
test('mastery needs spaced evidence from two activity types and activates remediation after an error', () => {
  ...
  const withError=projectActivities([event('a','2026-09-10T10:00:00Z',true),event('b','2026-09-11T11:00:00Z',true,'quiz'),event('c','2026-09-12T12:00:00Z',false)]);
  assert.equal(withError.mastery.objective.status,'mastered');
  assert.equal(withError.mastery.objective.remediationRequired,true);
  assert.equal(withError.mistakes.length,1);
  assert.equal(withError.mastery.objective.highConfidenceError,true);
});
